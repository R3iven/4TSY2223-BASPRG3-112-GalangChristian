...
{ 
	...
	texture = loadTexture("gfx/player.png");
		...
		shotgunSound = SoundManager::loadSound("sound/10 Guage Shotgun-SoundBible.com-74120584.ogg");
...
void Player::update()
{
	...
	if (app.keyboard[SDL_SCANCODE_F] && currentReloadTime == 0 )
	{
		...
		Bullet* bullet = new Bullet(x + width, y - 4 + height / 2, 1, 0, 15, Side::PLAYER_SIDE);
	...
	if (app.keyboard[SDL_SCANCODE_G] && currentReloadTime2 == 0)
	{
		...
		// spawn 1st bullet
		Bullet* bullet1 = new Bullet(x - 35 + width, y - 25 + height / 2, 1, 0, 10, Side::PLAYER_SIDE);
		bullets.push_back(bullet1);
		getScene()->addGameObject(bullet1);
		bullet1->start();

		// Spawn 2nd bullet
		Bullet* bullet2 = new Bullet(x - 35 + width, y + 15 + height / 2, 1, 0, 10, Side::PLAYER_SIDE);
