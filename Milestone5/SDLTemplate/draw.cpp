...
{
	SDL_Texture* backgroundTexture = loadTexture(backgroundFilename);

	// Tile the background image to fill the entire screen
	for (int x = 0; x < SCREEN_WIDTH; x += 512)
	{
		for (int y = 0; y < SCREEN_HEIGHT; y += 512)
		{
			blit(backgroundTexture, x, y);
		}
	}
}
